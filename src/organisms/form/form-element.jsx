...
const TextAreaField = (props) => <TextField {...props} label={props.label} multiline />;

const SelectField = (props) => {
    const { options } = props;
    const optionComponents = _map(options, ({name, value}) => <MenuItem key={`${name}-${Math.random}`} value={value}>{name}</MenuItem>);
    return <TextField {...props} label={props.label} select><MenuItem value=''>Selecionar...</MenuItem>{optionComponents}</TextField>;
};

const FIELD_COMPONENTS = {
    text: (props) => <TextField {...props} label={props.label} />,
